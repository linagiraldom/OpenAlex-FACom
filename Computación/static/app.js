...
var affiliation = url.searchParams.get("affiliation");
//
...
const APIurl = "http://127.0.0.1:8000/?affiliation="+affiliation;
...
function jsonList(item) {
  ...
  // div.innerHTML = `${item.affiliation} got Parcial 1: ${item.Parcial_1.value} from ${APIurl}`;
  div.innerHTML = `<span style='float: right; background-color:rgba(0,0,0,0.4);border-radius:4px;padding:4px;'>${count}</span> <br><b><p>Artículo</p></b> <p>${item[0]}</p><br><b><p>Institución</p> </b> <p>${item[1]}</p>`;
